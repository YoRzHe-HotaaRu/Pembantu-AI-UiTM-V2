...
Maps emotion tags from AI responses to Live2D expressions.
"""
...
class ExpressionConfig:
    ...
    emotion_tag: str        # Emotion tag (e.g., "HAPPY")
...
class ExpressionMapper:
    """
    Maps emotion tags to VTube Studio expressions.
    Supports both expression files and hotkey-based activation.
    """
    
    # Emotion tag pattern - matches [EMOTION] tags in AI responses
    EMOTION_PATTERN = re.compile(r'\[(NEUTRAL|HAPPY|SAD|ANGRY|SURPRISED|SHY|EXCITED|THINKING|CONFUSED|WORRIED)\]', re.IGNORECASE)
    ...
    DEFAULT_MAPPINGS: Dict[str, str] = {
        ...
        "CONFUSED": "confused.exp3.json",
        "WORRIED": "worried.exp3.json"
    }
    ...
    def remove_emotion_tag(self, text: str) -> str:
        """
        Remove emotion tag from text for clean display.
        
        Args:
            text: Text with potential emotion tag
            
        Returns:
            Text with emotion tag removed
        """
        return self.EMOTION_PATTERN.sub('', text).strip()
